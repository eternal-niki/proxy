    ...
    <body>
        <div class="wrap">
            <div class="card">
                ...
                <form id="proxyForm" action="/proxy" method="post" onsubmit="return prepareFormSubmit();">
                    ...
                    <div class="switch-row" style="justify-content:space-between;">
                        <div style="display:flex; align-items:center; gap:10px;">
                            <div id="toggle" class="switch on" onclick="toggleOriginalMeta()">
                                <div class="knob"></div>
                            </div>
        ...
        <script>
            // 初期状態: ON (元サイトの meta を使う)
            let originalMeta = true;
            function setToggleVisual() {
                const el = document.getElementById("toggle");
                if (originalMeta) el.classList.add("on");
                else el.classList.remove("on");
                document.getElementById("original_meta").value = originalMeta ? "true" : "false";
            }
            function toggleOriginalMeta() {
            ...
            function encodeBase64() {
                ...
                try {
                    ...
                    if (originalMeta) {
                        fullUrl += "&original_meta=true";
                    }
                    document.getElementById("b64_output").innerText = fullUrl;
                } catch(e) { alert("エンコードに失敗しました: " + e); }
            }

            // フォーム送信時に hidden の original_meta を更新（POST 用）
            function prepareFormSubmit() {
                document.getElementById("original_meta").value = originalMeta ? "true" : "false";
                // b64 は空でも OK — フォームの target_url を優先
                return true;
            }
        </script>
    </body>
...
def proxy():
    ...
    # original_meta 判定（GET クエリ or POST フォーム）
    ...
    try:
        ...
        if "text/html" in content_type:
            ...
            for tag in soup.find_all("a", href=True):
                ...
                if abs_url.startswith("http"):
                    ...
                    if req_type == "get":
                        ...
                        if encode_https:
                            new_href += "&encodetype=https"
                        if use_original_meta:
                            new_href += "&original_meta=true"
                        tag["href"] = new_href
                    else:
                        # POST で遷移するリンクは onclick で POST フォームを作る
                        tag["href"] = "#"
                        # use_original_meta を小文字 true/false で渡す
                        tag["onclick"] = f"proxyPost('{abs_b64}', {str(use_original_meta).lower()}); return false;"

            # form 書き換え（action -> /proxy, b64 hidden を入れる）
            ...
            # 画像/スクリプト/link/iframe の src/href をプロキシ経由へ
            for tag in soup.find_all(["img", "script", "link", "iframe"]):
                ...
                if tag.has_attr(attr):
                    ...
                    if abs_url.startswith("http"):
                        ...
                        if encode_https:
                            new_attr += "&encodetype=https"
                        if use_original_meta:
                            new_attr += "&original_meta=true"
                        tag[attr] = new_attr

            # meta 制御: use_original_meta が False のときに弐紀Webプロキシで偽装
            ...
            # injected script: proxyPost (POST 用) + ad remover（既存と同様）
            ...
            function proxyPost(b64, use_original_meta) {
                ...
                m.type = 'hidden';
                m.name = 'original_meta';
                m.value = use_original_meta ? 'true' : 'false';
                f.appendChild(m);
